...
def mergesort3(lst: list) -> list:
    ...
    You must implement a three-way mergesort, do not do a normal "merge" twice.
    ...
    >>> mergesort3([2, -20, 5, 6, 17, 10, 100, 101, 102, 103, 104])
    [-20, 2, 5, 6, 10, 17, 100, 101, 102, 103, 104]
    """
    if len(lst) < 2:
        return lst[:]
    elif len(lst) == 2:
        ...
        left_sorted = mergesort3(lst[:div1])
        mid_sorted = mergesort3(lst[div1:div2])
        right_sorted = mergesort3(lst[div2:])
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    while index1 < len(lst1) and index2 < len(lst2):
        if lst1[index1] <= lst2[index2]:
            pre_merged.append(lst1[index1])
            index1 += 1
        else:
            pre_merged.append(lst2[index2])
            index2 += 1
    pre_merged = pre_merged + lst1[index1:] + lst2[index2:]

    while pm_index < len(pre_merged) and index3 < len(lst3):
        if pre_merged[pm_index] <= lst3[index3]:
            merged.append(pre_merged[pm_index])
            pm_index += 1
        else:
            merged.append(lst3[index3])
            index3 += 1

    return merged + pre_merged[pm_index:] + lst3[index3:]
...
def kth_smallest(lst: list, k: int) -> Any:
    ...
    if k < 0 or k >= len(lst):
        ...
        if len(smaller) > k:
            ...
            return kth_smallest(bigger, k - len(bigger))
