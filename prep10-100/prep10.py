    ...
    def evaluate(self) -> Any:
        ...
        if self.op == 'and':
            ...
            for i in range(len(self.values) - 1):
                result = result and self.values[i + 1].evaluate()
            return result
        elif self.op == 'or':
            result = self.values[0].evaluate()
            for i in range(len(self.values) - 1):
                result = result or self.values[i + 1].evaluate()
            return result
        else:
            raise ValueError(f'Invalid operator {self.op}')
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        for i in range(len(self.comparisons)):
            exp = self.comparisons[i][0]
            right = self.comparisons[i][1].evaluate()
            if exp == '<':
                result = result and (left < right)
            elif exp == '<=':
                result = result and (left <= right)
            else:
                raise ValueError(f'Invalid operator {exp}')
            if not result:
                return result
            left = right
        return result
